...
const BranchesSection = ({ 
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0) {
        if (activeBranchPage + 1 < totalPages) onPageChange(activeBranchPage + 1);
      } else {
        if (activeBranchPage > 0) onPageChange(activeBranchPage - 1);
      }
    }
    setTouchStart(null);
    setDragOffset(0);
    ...
    <section className="pt-24 pb-12 md:pb-20 bg-[#F2F2F2] relative w-full overflow-hidden">
      <div className="container mx-auto px-4 relative z-10 max-w-[1500px]">
        ...
        <div className="flex items-center justify-center gap-4 md:gap-12 w-full max-w-[1700px] mx-auto relative px-4">
          <button
            onClick={() => onPageChange(Math.max(0, activeBranchPage - 1))}
            disabled={activeBranchPage === 0}
            className={`shrink-0 w-12 h-12 md:w-20 md:h-20 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-2xl border border-white/50 hover:bg-[#EE0033] group transition-all duration-500 z-30 ${activeBranchPage === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
          ...
          >
            <div
              ...
              style={{ transform: `translateX(calc(-${activeBranchPage * 100}% + ${dragOffset}px))` }}
            >
          ...
          <button
            onClick={() => onPageChange(activeBranchPage + 1 < totalPages ? activeBranchPage + 1 : activeBranchPage)}
            disabled={activeBranchPage + 1 >= totalPages}
            className={`shrink-0 w-12 h-12 md:w-20 md:h-20 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-2xl border border-white/50 hover:bg-[#EE0033] group transition-all duration-500 z-30 ${activeBranchPage + 1 >= totalPages ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
